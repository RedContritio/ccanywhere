...
- GIVEN ccanywhere server 运行在 7878，`pnpm -C web dev` 启动
- WHEN  浏览器访问 `http://localhost:5173/login`
- THEN  登录页正常加载
- AND   页面对 `/api/projects` 的请求被 Vite proxy 到 7878
...
当 viewport 满足 `pointer: coarse AND max-width: 768px` 时，前端 MUST 在
