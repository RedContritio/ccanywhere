#!/bin/bash
#
# Shared container manual verify — image 健康检查.
#
# 跑流程:
...
# 修订 (第二次): claude binary 通过 `npm install -g` 在 image
...
if [[ "$gh_code" != "200" ]]; then
  echo "FAIL: api.github.com unreachable (code $gh_code); over-blocking?" >&2
  exit 2
fi
echo "  OK: HTTP 200"

# 6. claude binary 调用
echo "[6/7] verifying claude binary (vendored via npm)..."
